      ...
      <div class="card-body">
        <div class="border-bottom mb-3 row">
            <div class="col-md-5" >
                ...
                <button onclick='return $("#add_modal").modal("show");' class="btn btn-inverse-primary btn-sm d-inline">{{__("Add Task")}}</button>
                @endif
                <a href="{{url('tasks')}}" class="btn btn-inverse-primary btn-sm d-inline">{{__("Switch To List")}}</a>
            </div>
                        ...
                        <ol data-draggable="target" id="not_started">
                            ...
                            <li data-draggable="item" id="not_started_{{$h->id}}">
                                ...
                                <div class="alert {{$li_color}}" role="alert" >
                                    <a  <?php  if(Auth::user()->hasPermissionTo('tasks.edit') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN)) { ?> onclick='edit_modal("{{$h->id}}")' <?php }?> ><b class="alert-heading">{{$h->name}}</b></a><br>
