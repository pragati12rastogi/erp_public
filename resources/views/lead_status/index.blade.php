...
@push('custom-scripts')
    <script>
        ...
        function edit_modal(edit_id){
            var submit_edit_url = '{{url("lead-status")}}/'+edit_id;
        ...
        function delete_modal(row_id){
            var delete_url = "{{url('lead-status')}}/"+row_id;
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="border-bottom mb-3 row">
            <div class="col-md-10">
                <h4 class="card-title">Lead Status Summary</h4>
            </div>
            @if(Auth::user()->hasPermissionTo('lead-status.create') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN) )
            <div class="col-md-2 text-end" >
              <button onclick='return $("#add_modal").modal("show");' class="btn btn-inverse-primary btn-sm">{{__("Add Status")}}</button>
            </div>
            @endif
        </div>
          ...
          <table id="status_table" class="table ">
            <thead>
              <tr>
                ...
                <th>Name</th>
                <th>Created At</th>
                @if(Auth::user()->hasPermissionTo('lead-status.edit') || Auth::user()->hasPermissionTo('lead-status.destroy') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN))
                <th>Action</th>
                @endif
              </tr>
            </thead>
            <tbody>
              @foreach($statuses as $key => $h)
                <tr>
                    <td>{{$key+1}}</td>
                    <td>{{$h->name}}</td>
                    ...
                    <td>
                      ...
                      @endif
                      @if((Auth::user()->hasPermissionTo('lead-status.destroy') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN)) && $h->is_deletable==0 )
...
@include('lead_status.form')
    ...
    <div id="delete_modal" class="delete-modal modal fade" role="dialog">
        ...
        <div class="modal-content">
            ...
            <p>Do you really want to delete this Status? This process cannot be undone.</p>
            </div>
