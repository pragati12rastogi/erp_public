...
@push('custom-scripts')
    <script>
        ...
        function edit_modal(edit_id){
            var submit_edit_url = '{{url("announcements")}}/'+edit_id;
            ...
            $.ajax({
                ...
                success:function(result){
                    ...
                    if(result != ''){
                        var inputs = result.announcement;
                        $('#announcement_form_upd').attr('action',submit_edit_url);
                        $('#c_name_upd').val(inputs.name);
                        var ck = CKEDITOR.instances['c_details_upd'].setData(inputs.message);
                        $("#user_ids_upd").val(inputs.user_ids).trigger('change');
        ...
        function delete_modal(row_id){
            var delete_url = "{{url('announcements')}}/"+row_id;
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="border-bottom mb-3 row">
            <div class="col-md-10">
                <h4 class="card-title">Announcement Summary</h4>
            </div>
            @if(Auth::user()->hasPermissionTo('announcements.create') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN) )
            <div class="col-md-2 text-end" >
              <button onclick='return $("#add_modal").modal("show");' class="btn btn-inverse-primary btn-sm">{{__("Add Announcement")}}</button>
            </div>
            @endif
        </div>
        
        <div class="table-responsive">
          <table id="announcement_table" class="table ">
            <thead>
              <tr>
                <th>Sr.no.</th>
                <th>Name</th>
                <th>Message</th>
                <th>Created At</th>
                @if(Auth::user()->hasPermissionTo('announcements.edit') || Auth::user()->hasPermissionTo('announcements.destroy') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN))
                <th>Action</th>
                @endif
              </tr>
            </thead>
            <tbody>
              @foreach($announcements as $key => $h)
                <tr>
                    <td>{{$key+1}}</td>
                    <td>{{$h->name}}</td>
                    <td>{!!$h->message!!}</td>
                    ...
                    <td>
                      ...
                      @endif
                      @if(Auth::user()->hasPermissionTo('announcements.destroy') || Auth::user()->hasRole(App\Custom\Constants::ROLE_ADMIN))
...
@include('announcement.form')
    ...
    <div id="delete_modal" class="delete-modal modal fade" role="dialog">
        ...
        <div class="modal-content">
            ...
            <p>Do you really want to delete this Announcement? This process cannot be undone.</p>
            </div>
